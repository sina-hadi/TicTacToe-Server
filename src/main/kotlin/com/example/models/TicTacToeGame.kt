...
class TicTacToeGame {
    ...
    // Specific data to specific client
    ...
    fun connectPlayer(session: WebSocketSession): Char? {
        val isPlayerX: Boolean = state.value.connectedPlayer.any { it == 'X' }
        val player = if (isPlayerX) 'O' else 'X'

        state.update {
            if (state.value.connectedPlayer.contains(player)) {
                return null // For Third player come to server
            }
